...
class DiagnosisResult(Fact):
    """
    Internal fact used by rules to store diagnosis conclusions.
    Not directly declared by user; created by expert system rules.
    """
    
    diagnosis_name = Field(str, mandatory=True)
    certainty_factor = Field(float, mandatory=True)
    severity_level = Field(int, mandatory=True)
    rule_name = Field(str, mandatory=True)
    reasoning = Field(str, default="")
